    ...
    public class WdlResampler
    {
        ...
        public void SetMode(bool interp, int filtercnt, bool sinc, int sinc_size = 64, int sinc_interpsize = 32)
        {
            ...
            m_sincoversize = m_sincsize != 0
                ? sinc_interpsize <= 1 ? 1 :
                sinc_interpsize >= 4096 ? 4096 : sinc_interpsize
                : 1;
            ...
            m_filtercnt = m_sincsize != 0
                ...
                : filtercnt <= 0
                    ? 0
                    : filtercnt >= WDL_RESAMPLE_MAX_FILTERS
            ...
            if (m_filtercnt == 0) m_iirfilter = null;
        }
        ...
        ///     Set feed mode
        /// </summary>
        /// <param name="wantInputDriven">
        ///     if true, that means the first parameter to ResamplePrepare will specify however much
        ///     input you have, not how much you want
        /// </param>
        ...
        /// <summary>
        ///     Prepare
        ///     note that it is safe to call ResamplePrepare without calling ResampleOut (the next call of ResamplePrepare will
        ///     function as normal)
        ...
        ///     req_samples is output samples desired if !wantInputDriven, or if wantInputDriven is input
        ...
        public int ResamplePrepare(int out_samples, int nch, out float[] inbuffer, out int inbufferOffset)
        {
            ...
            if (hfs > 1 && m_samples_in_rsinbuf < hfs - 1)
            {
                ...
                if (m_samples_in_rsinbuf > 0) m_rsinbuf = new float[m_samples_in_rsinbuf * nch];
            }
        ...
        public int ResampleOut(float[] outBuffer, int outBufferIndex, int nsamples_in, int nsamples_out,
            int nch)
        {
            ...
            else // linear interpolation
            {
                if (nch == 1)
                    while (ns-- != 0)
                    {
                        ...
                        if (ipos >= rsinbuf_availtemp - 1) break; // quit decoding, not enough input samples

                        var ifracpos = 1.0 - fracpos;
                        var inptr = localin + ipos;
                        outBuffer[outptr++] =
                            (float)(m_rsinbuf[inptr] * ifracpos + m_rsinbuf[inptr + 1] * fracpos);
                        srcpos += drspos;
                        ret++;
                    }
                else if (nch == 2)
                    ...
                    while (ns-- != 0)
                    {
                        ...
                        {
                            outBuffer[outptr++] =
                                (float)(m_rsinbuf[inptr] * ifracpos +
        ...
        private void BuildLowPass(double filtpos)
        {
                ...
                m_lp_oversize != wantinterp)
            {
                ...
                if (m_filter_coeffs.Length == allocsize)
                {
                    ...
                    var filtpower = 0.0;
                    var windowpos = 0.0;
                    var dwindowpos = 2.0 * PI / sz;
                    var
                        dsincpos = PI / m_lp_oversize *
                                   filtpos; // filtpos is outrate/inrate, i.e. 0.5 is going to half rate
                    var sincpos = dsincpos * -hsz;

                    int x;
                    for (x = -hsz; x < hsz + m_lp_oversize; x++)
                    {
                        var val = 0.35875 - 0.48829 * Math.Cos(windowpos) + 0.14128 * Math.Cos(2 * windowpos) -
        ...
        private void SincSample(float[] outBuffer, int outBufferIndex, float[] inBuffer,
            int inBufferIndex, double fracpos, int nch, float[] filter, int filterIndex, int filtsz)
        {
        ...
        private void SincSample1(float[] outBuffer, int outBufferIndex, float[] inBuffer,
            int inBufferIndex, double fracpos, float[] filter, int filterIndex, int filtsz)
        {
            var oversize = m_lp_oversize;
            fracpos *= oversize;
            var ifpos = (int)fracpos;
        ...
        private void SincSample2(float[] outptr, int outBufferIndex, float[] inBuffer,
            int inBufferIndex, double fracpos, float[] filter, int filterIndex, int filtsz)
        {
            var oversize = m_lp_oversize;
            fracpos *= oversize;
        ...
        private class WDL_Resampler_IIRFilter
        {
            ...
            public void Apply(float[] inBuffer, int inIndex, float[] outBuffer, int outIndex,
                int ns, int span, int w)
            {
                ...
                while (ns-- != 0)
                {
                    ...
                    inIndex += span;
                    var outx = inx * b0 + m_hist[w, 0] * b1 + m_hist[w, 1] * b2 - m_hist[w, 2] * a1 -
                               m_hist[w, 3] * a2;

    ...
    public class Mp3Frame
    {
        ...
        private static bool IsValidHeader(byte[] headerBytes, Mp3Frame frame)
        {
            if (headerBytes[0] == 0xFF && (headerBytes[1] & 0xE0) == 0xE0)
            {
                ...
                if (frame.MpegLayer == MpegLayer.Reserved) return false;
                ...
                if (frame.BitRate == 0) return false;

                var sampleFrequencyIndex = (headerBytes[2] & 0x0C) >> 2;
                if (sampleFrequencyIndex == 3) return false;
                ...
                if (frame.ChannelExtension != 0 && frame.ChannelMode != ChannelMode.JointStereo) return false;
                ...
                if (frame.FrameLength > MaxFrameLength) return false;

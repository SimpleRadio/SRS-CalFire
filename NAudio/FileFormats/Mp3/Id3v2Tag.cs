...
namespace NAudio.FileFormats.Mp3
{
    ...
    public class Id3v2Tag
    {
        ...
        {
            var size = 0;
            foreach (var frame in frames) size += frame.Length;
        ...
        private static Stream CreateId3v2TagStream(IEnumerable<KeyValuePair<string, string>> tags)
        {
            ...
            foreach (var tag in tags) frames.Add(CreateId3v2Frame(tag.Key, tag.Value));

            var header = CreateId3v2TagHeader(frames);

            var ms = new MemoryStream();
            ms.Write(header, 0, header.Length);
            foreach (var frame in frames) ms.Write(frame, 0, frame.Length);

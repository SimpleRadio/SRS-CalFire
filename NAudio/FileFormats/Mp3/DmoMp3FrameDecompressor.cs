...
namespace NAudio.FileFormats.Mp3
{
    ...
    public class DmoMp3FrameDecompressor : IMp3FrameDecompressor
    {
        ...
        public DmoMp3FrameDecompressor(WaveFormat sourceFormat)
        {
            ...
            OutputFormat = new WaveFormat(sourceFormat.SampleRate, sourceFormat.Channels); // 16 bit
            if (!mp3Decoder.MediaObject.SupportsOutputWaveFormat(0, OutputFormat))
                throw new ArgumentException(string.Format("Unsupported output format {0}", OutputFormat));

            mp3Decoder.MediaObject.SetOutputWaveFormat(0, OutputFormat);

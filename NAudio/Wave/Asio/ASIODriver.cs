...
namespace NAudio.Wave.Asio
{
    ...
    public class AsioDriver
    {
        ...
        public static AsioDriver GetAsioDriverByName(string name)
        {
            ...
            if (regKey == null) throw new ArgumentException($"Driver Name {name} doesn't exist");
        ...
        public bool CanSampleRate(double sampleRate)
        {
            ...
            if (error == AsioError.ASE_NoClock) return false;

            if (error == AsioError.ASE_OK) return true;
        ...
        private void HandleException(AsioError error, string methodName)
        {
            if (error != AsioError.ASE_OK && error != AsioError.ASE_SUCCESS)
            {
                var asioException = new AsioException(
                    $"Error code [{AsioException.getErrorName(error)}] while calling ASIO method <{methodName}>, {GetErrorMessage()}");
        ...
        private void InitFromGuid(Guid asioGuid)
        {
            ...
            var hresult = CoCreateInstance(ref asioGuid, IntPtr.Zero, CLSCTX_INPROC_SERVER, ref asioGuid,
                out pAsioComObject);
            if (hresult != 0) throw new COMException("Unable to instantiate ASIO. Check if STAThread is set", hresult);

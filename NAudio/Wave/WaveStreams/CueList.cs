...
namespace NAudio.Wave
{
    ...
    public class CueList
    {
        ...
        internal CueList(byte[] cueChunkData, byte[] listChunkData)
        {
            ...
            for (var i = 0; i < cueCount; i++) cues.Add(new Cue(positions[i], labels[i]));
        }
        ...
        public int[] CuePositions
        {
            get
            {
                ...
                for (var i = 0; i < cues.Count; i++) positions[i] = cues[i].Position;
        ...
        public string[] CueLabels
        {
            get
            {
                ...
                for (var i = 0; i < cues.Count; i++) labels[i] = cues[i].Label;
        ...
        internal static CueList FromChunks(WaveFileReader reader)
        {
            ...
            foreach (var chunk in reader.ExtraChunks)
                ...
                else if (chunk.IdentifierAsString.ToLower() == "list") listChunkData = reader.GetChunkData(chunk);

            if (cueChunkData != null && listChunkData != null) cueList = new CueList(cueChunkData, listChunkData);

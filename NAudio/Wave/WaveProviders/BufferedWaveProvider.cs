...
namespace NAudio.Wave
{
    ...
    public class BufferedWaveProvider : IWaveProvider
    {
        ...
        public void AddSamples(byte[] buffer, int offset, int count)
        {
            ...
            if (circularBuffer == null) circularBuffer = new CircularBuffer(BufferLength);

            var written = circularBuffer.Write(buffer, offset, count);
            if (written < count && !DiscardOnBufferOverflow) throw new InvalidOperationException("Buffer full");
        }

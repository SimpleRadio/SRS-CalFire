...
namespace NAudio.Wave.WaveOutputs
{
    ...
    public class AsioOut : IWavePlayer
    {
        ...
        public AsioOut(int driverIndex)
        {
            ...
            if (names.Length == 0) throw new ArgumentException("There is no ASIO Driver installed on your system");
        ...
        public void Dispose()
        {
            if (driver != null)
            {
                if (PlaybackState != PlaybackState.Stopped) driver.Stop();
        ...
        public void InitRecordAndPlayback(IWaveProvider waveProvider, int recordChannels, int recordOnlySampleRate)
        {
            ...
            if (driver.Capabilities.SampleRate != desiredSampleRate) driver.SetSampleRate(desiredSampleRate);

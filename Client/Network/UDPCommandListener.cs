    ...
    public class UDPCommandListener
    {
        ...
        private void StartUDPCommandListener()
        {
            while (!_stop)
            {
                ...
                try
                {
                    var localEp = new IPEndPoint(IPAddress.Any, _globalSettings.GetNetworkSetting(GlobalSettingsKeys.UDPCommandListenerPort));
                    _udpCommandListener = new UdpClient(localEp);
                    break;
                }
                catch (Exception ex)
                {
                    Logger.Warn(ex, $"Unable to bind to the UDP Command Listener Socket Port: {_globalSettings.GetNetworkSetting(GlobalSettingsKeys.UDPCommandListenerPort)}");
                    Thread.Sleep(500);
                }
            }
            ...
            while (!_stop)
            {
                try
                {
                    ...
                    else if (message?.Command == UDPInterfaceCommand.VOLUME)
                    {
                        ...
                        else if (message?.Parameter == 0)
                        {
                            RadioHelper.SetRadioVolume(0, message.RadioId);
                        }
                    }
                    else
                    {
                ...
                catch (SocketException e)
                {
                    ...
                    if (!_stop)
                    {
                        Logger.Error(e, "SocketException Handling UDP Message");
                    }
                }
                catch (Exception e)
                {

    ...
    internal class UdpVoiceHandler
    {
        ...
        private void RetransmitAudio(UDPVoicePacket udpVoicePacket, List<RadioReceivingPriority> radioReceivingPriorities)
        {
            ...
            var receivingWithRetransmit = radioReceivingPriorities.Where(receivingRadio =>
                ...
                //check global
                && !globalFrequencies.Any(freq => PlayerRadioInfo.FreqCloseEnough(receivingRadio.ReceivingRadio.freq, freq))
            ...
            var radiosWithRetransmit = _clientStateSingleton.PlayerRadioInfo.radios.Where(radio => radio.retransmit);

            //Check we're not retransmitting through a radio we just received on?
            foreach (var receivingRadio in receivingWithRetransmit)
            {
                radiosWithRetransmit = radiosWithRetransmit.Where(radio => !PlayerRadioInfo.FreqCloseEnough(radio.freq, receivingRadio.Frequency));
            }
        ...
        private List<int> CurrentlyBlockedRadios()
        {
            ...
            if (_clientStateSingleton.PlayerRadioInfo.simultaneousTransmission)
            {
                // Skip intercom
                for (int i = 1; i < 11; i++)
                {
                    var radio = _clientStateSingleton.PlayerRadioInfo.radios[i];
                    if ( (radio.modulation == RadioInformation.Modulation.FM || radio.modulation == RadioInformation.Modulation.AM )&& radio.simul &&
                        i != _clientStateSingleton.PlayerRadioInfo.selected)
                    {
                        transmitting.Add(i);
                    }
                }
        ...
        private List<RadioInformation> PTTPressed(out int sendingOn)
        {
            ...
            if (_ptt || _clientStateSingleton.PlayerRadioInfo.ptt)
            {
                ...
                if (_clientStateSingleton.PlayerRadioInfo.simultaneousTransmission)
                {
                    ...
                    if (currentSelected == 0 && currentlySelectedRadio.modulation == Modulation.INTERCOM && _clientStateSingleton.PlayerRadioInfo.inAircraft == false)
                    {
                    ...
                    var i = 0;
                    foreach (var radio in _clientStateSingleton.PlayerRadioInfo.radios)
                    {

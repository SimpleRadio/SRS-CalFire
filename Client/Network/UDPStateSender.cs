...
namespace Ciribob.FS3D.SimpleRadio.Standalone.Client.Network
{
    public class UDPStateSender
    {
        ...
        {
            IPEndPoint endPoint = new IPEndPoint(IPAddress.Loopback,_globalSettings.GetClientSetting(GlobalSettingsKeys.UDPExternalSenderPort).IntValue);
            ...
            while (!_stop)
            {
                ...
                catch (SocketException e)
                {
                    ...
                    if (!_stop)
                    {
                        Logger.Error(e, "SocketException Sending UDP State Message");
                    }
                }
                catch (Exception e)
                {
        ...
        private UDPExternalStateMessage GenerateExternalStateMessage()
        {
            ...
            List<UDPExternalRadioState> radios = new List<UDPExternalRadioState>();
            ...
            foreach(var radio in ClientStateSingleton.Instance.PlayerUnitState.Radios)
            {
                ...
                if (radio.CurrentChannel != null)
                {
                    externalRadio.Chn = radio.CurrentChannel.Channel;
                }
